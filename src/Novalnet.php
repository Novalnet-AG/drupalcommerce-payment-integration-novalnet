...
class Novalnet {
  ...
  private static $dataParams = ['auth_code', 'product', 'tariff', 'amount',
   ...
   public static function getMerchantDetails($request) {
     ...
     */
    public static function getCustomerData($order, $allow_b2b = null) {
        $payment = $order->get('payment_gateway')->entity->getPluginId();
        $config                                 = \Drupal::config('commerce_novalnet.application_settings');
        ...
        if ($customer_data['billing'] != $shipping_address) {
          $customer_data['shipping'] =  $shipping_address;
        }
        else {
          $customer_data['same_as_billing'] = 1;
        }
     ...
     */
    public static function getTransactionData($order, $payment, $configuration) {
        $code = $order->get('payment_gateway')->entity->getPluginId();
        $transaction_data = [
	...
	*/
	 ...
	 * @return string
	 */
	public static function checkGuaranteeProcess($order, $payment, array $guarantee_configuartion, $birth_date='') {
		if ($guarantee_configuartion['guarantee_configuration'][$payment . '_guarantee_payment']) {
		  ...
		  if (self::formatAmount($order->getTotalPrice()->getNumber()) < (int) $minimum_amount) {
			$message .= t('Minimum order amount must be ' . $minimum_amount . ' ' .'Є'. '<br>');
		  }

		  if (empty($birth_date[$payment . '_dob'])) {
			$message .= t('Please enter your date of birth');
		  }
		  elseif (time() < strtotime('+18 years', strtotime($birth_date[$payment . '_dob']))) {
  ...
  public static function includeFiles(array &$form, $payment, $library) {
	...
	*/
	public static function formBankDetails(array $response) {
		...
		$bank_details = '<br/><div id=nn_bank_details>'.
		...
		if($response['transaction']['status_code'] == 100) {
		  $bank_details .= t('Due date: @due_date', ['@due_date' => $response['transaction']['due_date']]) . '<br/>';
		}
		$bank_details .= t('Account holder: @holder', ['@holder' => $response['transaction']['bank_details']['account_holder']]) . '<br/>' .
		...
		$bank_details .= '<br/>'.t('Please use any one of the following references as the payment reference, as only through this way your payment is matched and assigned to the order:').'<br/>';
	 ...
	 * @return string
	 */
	public static function cancellation(array $response, $order_id, $payment_method) {
		global $base_url;
		if (in_array($payment_method, self::$redirectPayments)) {
